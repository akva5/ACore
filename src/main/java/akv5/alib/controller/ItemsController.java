...
public class ItemsController {
    ...
    public static ItemStack itemMove(InventoryDragEvent e) {
        ...
        Scheduler.doAsync(() ->
                e.getNewItems().forEach((slot, item) ->
                        Informer.sendInfo(Tools.join(" ", NMS.Inventory.getItem(e.getView(), slot), "MOVE TO", slot < e.getInventory().getSize() ? "OUTSIDE" : "INSIDE"))));
        return null;
    }
    ...
    public static boolean isEmptyBundle(ItemStack item) {
        ...
        try {
            ...
            ItemMeta meta = item.getItemMeta();
            if (meta != null && bundleMetaClass.isInstance(meta)) {
                java.lang.reflect.Method getItemsMethod = bundleMetaClass.getMethod("getItems");
                Object items = getItemsMethod.invoke(meta);
