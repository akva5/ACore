package akv5.alib.wrapper.packet.bukkit;
...
public abstract class AbstractPacket {
    ...
    protected AbstractPacket(PacketContainer handle, PacketType type) {
        ...
        if (!Objects.equal(handle.getType(), type))
            throw new IllegalArgumentException(handle + " is not a packet of type " + type);
        this.handle = handle;
    }
    ...
    public void setHandle(PacketContainer handle) {
        this.handle = handle;
    }
    ...
    public void sendPacket(Player receiver) {
        try {
            ...
            Informer.sendInfo("Cannot send packet: " + getHandle().getType().name());
            t.printStackTrace();
        }
    }
