...
import akv5.alib.file.util.Validate;
...
public class ConfigurationSerialization {
    ...
    private static final Map<String, Class<? extends ConfigurationSerializable>> aliases = new ConcurrentHashMap<>();
    ...
    protected ConfigurationSerializable deserializeViaMethod(@NotNull Method method, @NotNull Map<String, ?> args) {
        try {
            ...
            Logger.getLogger(ConfigurationSerialization.class.getName()).log(Level.SEVERE, "Could not call method '" + method + "' of " + clazz + " for deserialization", ex instanceof InvocationTargetException ? ex.getCause() : ex);
        }
    ...
    protected ConfigurationSerializable deserializeViaCtor(@NotNull Constructor<? extends ConfigurationSerializable> ctor, @NotNull Map<String, ?> args) {
        try {
            ...
            Logger.getLogger(ConfigurationSerialization.class.getName()).log(Level.SEVERE, "Could not call constructor '" + ctor + "' of " + clazz + " for deserialization", ex instanceof InvocationTargetException ? ex.getCause() : ex);
        }
        return null;
    }
    ...
    public ConfigurationSerializable deserialize(@NotNull Map<String, ?> args) {
        ...
        if (method != null) result = deserializeViaMethod(method, args);
    ...
    public static String getAlias(@NotNull Class<? extends ConfigurationSerializable> clazz) {
        ...
        if (delegate != null && delegate.value() != clazz) return getAlias(delegate.value());
