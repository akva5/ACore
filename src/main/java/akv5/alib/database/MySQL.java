...
public class MySQL {
    ...
    public MySQL(String address, String database, String username, String password, String url, String driver, Integer keepaliveTime, Integer connectionTimeout, Integer maxLifetime, Integer poolSize) {
        ...
        try {
            ...
            Informer.sendInfo("MySQL successfully connected!");
        } catch (Throwable t) {
            Informer.sendWarn("Failed to connect to the MySQL! (" + jdbc + ")");
            t.printStackTrace();
            setEnabled(false);
    ...
    private void handleSQLException(SQLException ex, String sql, Object[] args) {
        Informer.sendWarn("SQL error executing query: " + sql);
        if (args != null && args.length > 0) Informer.sendWarn("SQL args: " + IntStream.range(0, args.length)
                .mapToObj(i -> i + " => " + args[i])
                .reduce((s1, s2) -> s1 + ", " + s2)
                .orElse(""));
        ex.printStackTrace();
